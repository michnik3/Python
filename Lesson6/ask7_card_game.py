...
while active_game:
    ...
    # read first position
    first_position = int(input("Give the first position(0-(n-1)) and closed: "))
    while (first_position < 0 or first_position >= N) or state[first_position] == "open":
        print("Error!")
        first_position = int(input("Give the first position(0-(n-1))and closed: "))

    # read second position
    second_position = int(input("Give the first position(0-(n-1)) and closed: "))
    ...
    # check if the same open, else closed
